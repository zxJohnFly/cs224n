...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    x = x / np.sqrt(np.sum(x*x, axis=1)[:, None])
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    # 1. P( W_O | W_I )
    preds = softmax(np.dot(outputVectors, predicted))
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    word2vecCostAndGradient -- the cost and gradient fungction for
    ...
    vhat = inputVectors[currentWordIndex]

    for w in contextWords:
        w_index = tokens[w]
        w_cost, w_grad_in, w_grad_out = \
            word2vecCostAndGradient(vhat, w_index, outputVectors, dataset)
        cost += w_cost
        gradIn[currentWordIndex] += w_grad_in
        gradOut += w_grad_out
    ### END YOUR CODE
